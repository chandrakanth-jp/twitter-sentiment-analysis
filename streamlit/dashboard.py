...
def load_data(client,collection_name):
        ...
        collection: str
                name of collection
        """
        all_records = list(client[config('DB_NAME')][collection_name].find())
...
def sentiment_plot_data(df, freq):
        ...
        plot_df = df.set_index('created_at').groupby('subject').resample(freq)['sentiment.score'].mean().unstack(level=0, fill_value=0)
...
option = st.sidebar.selectbox(label='Select Collection from database', options=mongo_client[config('DB_NAME')].list_collection_names(),
                                index=0)
if option:
...
def stream_tweets(collection,duration, search_term):
        ...
        if search_term and duration:
                ...
                myStreamListener = MyStreamListener(keywords, collection, config('BOOTSTRAP_SERVER'))
...
def form_prompt_submit(collection):
        ...
        with st.sidebar:
                ...
                duration = st.selectbox(label='Duration (minutes)', options=[1,5,10])
...
if collection:
        user_form_input(mongo_client[config('DB_NAME')], collection)
